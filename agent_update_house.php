...
<html>
    ...
    <body>
        ...
        <div class="container col-md-10 col-10 col-lg-10" style="top:10%;">
            <h1 class="title">Update House</h1>
            ...
            <?php 
                if (isset($_POST['filterResaleId'])){
                    ...
                    $sql = mysqli_query($conn, "SELECT resaleId FROM resale_putup WHERE sellerId = \"".$_SESSION['sellerId']."\""); 
                    if ($searchResaleId == NULL){
                        ...
                        if ($count++ > 0){
                            $query = mysqli_query($conn,'SELECT * FROM resale_putup WHERE resaleId ='.$searchResaleId.' AND sellerId = "'.$_SESSION['sellerId'].'"') or die(mysqli_error($conn));
                            $num = mysqli_num_rows($query);
                        }
                        else{
                            $num = 0;
                    ...
                    echo '<table style="font-family: arial, sans-serif; border-collapse: collapse; width: 100%;">
                        ...
                        while($row = mysqli_fetch_assoc($query)):
                            echo "<tr style=\"background-color: #ffffff;\">";
                                ...
                                echo "<td style=\"border: 1px solid #dddddd; text-align: left; padding: 8px;\">".$row['available']."</td>";
                                echo "<td>";
                                    echo "<form method=\"POST\" style=\"margin:auto; text-align:center;\">";
                                        echo "<input type=\"hidden\" name=\"updateResaleId\" value=\"".$row['resaleId']."\">";
                                        if($row['available'] == 0){
                                            echo "<input type=\"submit\" style=\"display:inline-block; height:100%;\" class=\"button\" name=\"+\" value=\"+\">";
                                        }
                                        if($row['available'] == 1){
                                            echo "<input type=\"submit\" style=\"display:inline-block; height:100%;\" class=\"button\" name=\"-\" value=\"-\">";
                                        }
